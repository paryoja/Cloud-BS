...
public class WithoutAlignMapper extends Mapper<LongWritable, Text, Text, Text> {
	...
	public void setup( Context context ) throws IOException, InterruptedException {
		bypass = context.getConfiguration().getBoolean( "bypass", false );
	}
	...
	public void cleanup( Context context ) throws IOException, InterruptedException {
		...
		Process pr = rt.exec( "mkdir /tmp/bypass" );
		...
		String ctInputPath = "/tmp/bypass/iv_C2T_" + taskId + ".fa";
		String gaInputPath = "/tmp/bypass/iv_G2A_" + taskId + ".fa";

		System.out.println( "[DEBUG] bypass: " + bypass + " " + ctInputPath + " " + gaInputPath );
		...
		if( !bypass ) {
			runBowtie2( refPath, "W_C2T", ctInputPath, "/tmp/bypass/" + "W_C2T_" + taskId + ".sam" );
			runBowtie2( refPath, "C_C2T", ctInputPath, "/tmp/bypass/" + "C_C2T_" + taskId + ".sam" );
			runBowtie2( refPath, "W_G2A", gaInputPath, "/tmp/bypass/" + "W_G2A_" + taskId + ".sam" );
			runBowtie2( refPath, "C_G2A", gaInputPath, "/tmp/bypass/" + "C_G2A_" + taskId + ".sam" );

			emitResult( context, "/tmp/bypass/" + "W_C2T_" + taskId + ".sam", "W_C2T" );
			emitResult( context, "/tmp/bypass/" + "C_C2T_" + taskId + ".sam", "C_C2T" );
			emitResult( context, "/tmp/bypass/" + "W_G2A_" + taskId + ".sam", "W_G2A" );
			emitResult( context, "/tmp/bypass/" + "C_G2A_" + taskId + ".sam", "C_G2A" );
		}
		else {
			emitExistResult( context, "/tmp/bypass/", "W_C2T" );
			emitExistResult( context, "/tmp/bypass/", "C_C2T" );
			emitExistResult( context, "/tmp/bypass/", "W_G2A" );
			emitExistResult( context, "/tmp/bypass/", "C_G2A" );
		}
	...
	public void emitExistResult( Context context, String folderPath, String method ) throws IOException, InterruptedException {

		File folder = new File( folderPath );
		for( final File fileEntry : folder.listFiles() ) {
			if( !fileEntry.isDirectory() ) {
				if( fileEntry.getName().startsWith( method ) ) {

					File emitted = new File( fileEntry.getPath() + ".emit" );
					if( emitted.exists() || fileEntry.getPath().endsWith( ".emit" ) ) {
						continue;
					}
					...
					BufferedWriter bw = new BufferedWriter( new FileWriter( fileEntry.getPath() + ".emit" ) );
					bw.write( "emitted " + fileEntry.getPath() );
					bw.close();
				}
			}
		}
