...
#  out_path = os.path.join( "/data/project/SparkMethyl/test", "after_align" )
  ...
  in_path = os.path.join( "/home/hadoop/bio", "data_genomes" )
  out_path = os.path.join( "/home/hadoop/bio", "after_align" )
  hdfs_out_path = os.path.join( "/user/hadoop/", "after_align" )
...
def align(sc, argv):
  ...
  wc2t_seqs = c2t_seqs.mapPartitionsWithIndex( lambda i, ptn: run_bowtie2_and_get(i, "C2T", ["W_C2T", "C_C2T"], ptn, ref_path), True ).persist(StorageLevel.MEMORY_AND_DISK)
  wg2a_seqs = g2a_seqs.mapPartitionsWithIndex( lambda i, ptn: run_bowtie2_and_get(i, "G2A", ["W_G2A", "C_G2A"], ptn, ref_path), True ).persist(StorageLevel.MEMORY_AND_DISK)
