...
if TEST:
    #  ref_path = os.path.join( "/data/project/SparkMethyl/test", "test_ref_genomes" )
    #  in_path = os.path.join( "/data/project/SparkMethyl/test", "data_genomes" )
    #  out_path = os.path.join( "/data/project/SparkMethyl/test", "after_align" )
    ...
    in_path = os.path.join("/home/hadoop/bio", "data_genomes")
    out_path = os.path.join("/home/hadoop/bio", "after_align")
    hdfs_out_path = os.path.join("/user/hadoop/", "after_align_from_bowtie")
...
def align(sc, argv):
    ...
    ## the biggest data
    i_file = 'bowtie/output'

    ref_path = '/home/hadoop/bio/chr_ref_genomes'

    ## broadcast raw reference genome
    in_seqs = sc.newAPIHadoopFile(i_file, "org.apache.hadoop.mapreduce.lib.input.TextInputFormat",
    ...
    # joined_seqs.saveAsTextFile( os.path.join( argv[1], "joined"))
    ...
    # .saveAsTextFile( os.path.join( hdfs_out_path, argv[1] + "res"))
...
###
def select_and_find_uniq_alignment(string):
    methods = {"W_C2T": 0, "C_C2T": 1, "W_G2A": 2, "C_G2A": 3}
    group = [[], [], [], []]  # each index is method index
    raw = None

    split = string.encode('ascii', 'ignore').split(':', 1)
    raw = split[0]

    lst = re.findall("\"([^\"]+)\"", split[1])
    for el in lst:
        elem = re.sub('\s+', ' ', re.sub('\(|\)', '', el)).split(' ')
        elem[1] = int(elem[1])
        # print( elem )
...
def calc_methyl_partitions(ptn):
    ...
    for (chrm_name, d) in ptn:
        if curr_chrm_name != chrm_name:
            print
            "[INFO] chrm change from %s to %s" % (curr_chrm_name, chrm_name)
...
def calc_methyl(pair, ref_chrm, ref_length):
    ...
    return (read_id, (mismatches, method, chrm, target_strand, start_pos, cigar_str, target_seq, methy,
                      "%s_%s_%s" % (prev2_seq, g_aln, next2_seq), uniq))
        ...
        for line in fp:
            try:
                ...
                if not unmapped:
                    ...
                    yield (QNAME.encode('ascii', 'replace'),
                           (method, mismatches, RNAME, int(POS) - 1, CIGAR.encode('ascii', 'replace')))
            except ValueError:
                print
                filename
                print
